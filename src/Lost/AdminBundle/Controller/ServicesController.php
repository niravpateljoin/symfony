...
use Lost\AdminBundle\Form\Type\ServicesFormType;
...
class ServicesController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        //Check permission
        if(! $this->get('admin_permission')->checkPermission('service_list')) {
        ...
        $query = $em->getRepository('LostUserBundle:Service')->getAllServices();
        $paginator = $this->get('knp_paginator');
        $pagination = $paginator->paginate($query, $request->query->get('page', 1), 10);
        return $this->render('LostAdminBundle:Services:index.html.twig', array('pagination' => $pagination));
    }
    ...
    public function serviceListJsonAction($orderBy = "id", $sortOrder = "asc", $search = "all", $offset = 0) {
        ...
        $data  = $em->getRepository('LostUserBundle:Service')->getServiceGridList($per_page, $offset, $orderBy, $sortOrder, $gridData['search_data'], $gridData['SearchType'], $helper);
        ...
        if (isset($data) && !empty($data)) {
            ...
            if (isset($data['result']) && !empty($data['result'])) {
                ...
                foreach ($data['result'] AS $resultRow) {
                    ...
                    $row = array();
                    $row[] = $resultRow->getId();
                    $row[] = '<span class="btn btn-success btn-sm">'.$resultRow->getName().'</span>';
    ...
    public function newAction(Request $request) {
        ...
        return $this->render('LostAdminBundle:Services:new.html.twig', array('form' => $form->createView()));        
    }
    ...
    public function editAction(Request $request, $id) {
        ...
        $admin = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $services = $em->getRepository('LostUserBundle:Service')->find($id);
        ...
        return $this->render('LostAdminBundle:Services:edit.html.twig', array('form' => $form->createView(), 'services' => $services));        
    }
    ...
    public function deleteAction(Request $request) {
        ...
        if(! $this->get('admin_permission')->checkPermission('service_delete')) {
            $this->get('session')->getFlashBag()->add('failure', "You are not allowed to delete service.");
            return $this->redirect($this->generateUrl('lost_admin_dashboard'));
        }
