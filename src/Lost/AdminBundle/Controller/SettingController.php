...
use Lost\AdminBundle\Form\Type\SettingFormType;
use Lost\AdminBundle\Entity\Setting;
...
class SettingController extends Controller {
    ...
    public function indexAction(Request $request) {
        ...
        //Check permission
        if(! $this->get('admin_permission')->checkPermission('settings_list')) {
        ...
        if ($admin) {
            ...
            $query = $em->getRepository('LostAdminBundle:Setting')->getAllSetting();

            $paginator = $this->get('knp_paginator');
            $pagination = $paginator->paginate($query, $request->query->get('page', 1), 10); 
            return $this->render('LostAdminBundle:Setting:index.html.twig', array('pagination' => $pagination, 'role' => $roles[0]));
        } else {

            $this->get('session')->getFlashBag()->add('failure', "You are not allowed to view settings!");
            return $this->redirect($this->generateUrl('lost_admin_setting_list'));
        }
    }
    ...
    public function settingListJsonAction($orderBy = "id", $sortOrder = "asc", $search = "all", $offset = 0) {
        ...
        $data  = $em->getRepository('LostAdminBundle:Setting')->getSettingGridList($per_page, $offset, $orderBy, $sortOrder, $gridData['search_data'], $gridData['SearchType'], $helper);
        ...
        if (isset($data) && !empty($data)) {
            ...
            if (isset($data['result']) && !empty($data['result'])) {
                ...
                foreach ($data['result'] AS $resultRow) {
                    ...
                    $name = '<a href="'.$this->generateUrl('lost_admin_setting_edit', array('id' => $resultRow->getId())).'">'.$resultRow->getName().'</a>';
                    $row = array();
                    $row[] = $resultRow->getId();
                    $row[] = $name;
    ...
    public function newAction(Request $request) {
        ...
        return $this->render('LostAdminBundle:Setting:new.html.twig', array('form' => $form->createView()));
    }
    ...
    public function editAction(Request $request, $id) {
        ...
        $admin = $this->get('security.context')->getToken()->getUser();
        $em = $this->getDoctrine()->getManager();
        $setting = $em->getRepository('LostAdminBundle:Setting')->find($id);
        ...
        return $this->render('LostAdminBundle:Setting:edit.html.twig', array('form' => $form->createView(), 'setting' => $setting));
    }
    ...
    public function deleteAction(Request $request) {
      //  $id = $request->get('id');
        ...
        if(! $this->get('admin_permission')->checkPermission('settings_delete')) {
            $this->get('session')->getFlashBag()->add('failure', "You are not allowed to delete settings.");
            return $this->redirect($this->generateUrl('lost_admin_dashboard'));
        }
        ...
        $objSetting = $em->getRepository('LostAdminBundle:Setting')->find($request->get('id'));
